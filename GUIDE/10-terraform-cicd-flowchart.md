...
10. [Flowchart (Terraform + CI/CD)](10-terraform-cicd-flowchart.md)
...
flowchart TD
    ...
    subgraph TERRAFORM_STAGE ["STAGE 1 — Terraform: Build Cloud Infrastructure"]
        ...
        Storage buckets, BigQuery tables,
    ...
    This single push triggers GitHub Actions."]

    GIT_COMMIT --> GIT_REPO[("Git Repository
    Now contains:
    terraform-output.json and all ML code.
    No secrets inside.")]
    ...
    subgraph CICD_STAGE ["STAGE 2 — CI/CD: Test, Run and Deploy"]
        ...
        terraform-output.json, GCP_SA_KEY, ACTIVE_ENV"]]

        GHA --> JOB_TEST["JOB 1 — Run Tests  —  < pytest tests/ >
        ─────────────────────────────────────────────────
        ...
        JOB_PROMOTE -- "Yes — auto-merge to next branch" --> MERGE["Merge to next branch
        e.g. dev to qa
    ...
    MERGE -- "triggers CI/CD for next environment" --> GHA
    ...
    linkStyle default stroke:#222222,stroke-width:2.5px
    ...
    class START,CODE_BRANCH neutral
```
